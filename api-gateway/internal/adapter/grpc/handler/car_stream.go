	...
	for {
		...
		if err != nil {
			...
			if isUnavailable(err) {
				log.Warn("transient error opening car filter stream, reconnecting", pkglog.Err(err))
				select {
		...
		for {
			...
			if err != nil {
				...
				if isUnavailable(err) {
					log.Warn("car filter stream interrupted, reconnecting", pkglog.Err(err))
					select {
					case <-time.After(streamReconnectDelay):
					case <-ctx.Done():
						return nil
...
func (h *CarHandler) StreamCarStatusUpdates(ctx context.Context, carID string, send func(model.CarStatusEvent) error) error {
	...
	for {
		...
		stream, err := h.streamClient.StreamCarStatusUpdates(ctx, &carsvc.StreamCarStatusUpdatesRequest{CarId: carID})
		if err != nil {
			if ctx.Err() != nil {
				return nil
			}
			if isUnavailable(err) {
				log.Warn("transient error opening car status stream, reconnecting", pkglog.Err(err))
				select {
				case <-time.After(streamReconnectDelay):
				case <-ctx.Done():
					return nil
		...
		for {
			...
			if err != nil {
				...
				if isUnavailable(err) {
					log.Warn("car status stream interrupted, reconnecting", pkglog.Err(err))
					select {
					case <-time.After(streamReconnectDelay):
					case <-ctx.Done():
						return nil
...
func (h *CarHandler) StreamCarTelemetry(ctx context.Context, carID string, send func(model.CarTelemetryEvent) error) error {
	...
	for {
		...
		if err != nil {
			...
			if isUnavailable(err) {
				log.Warn("transient error opening car telemetry stream, reconnecting", pkglog.Err(err))
				select {
				case <-time.After(streamReconnectDelay):
				case <-ctx.Done():
					return nil
		...
		for {
			...
			if err != nil {
				...
				if isUnavailable(err) {
					log.Warn("car telemetry stream interrupted, reconnecting", pkglog.Err(err))
					select {
					case <-time.After(streamReconnectDelay):
					case <-ctx.Done():
						return nil
